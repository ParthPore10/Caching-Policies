...
LFUCache::LFUCache(int capJamLimit) : capJam(capJamLimit) {
    hits = 0;
    total_hits = 0;
    calls = 0;
    total_calls = 0;
    ...
    std::cout << "Cache size is: " << capJam << std::endl;
}
...
void LFUCache::refer(long long int key, string rwtype) {
    ...
    if (key_to_freq.find(key) == key_to_freq.end()) {
        ...
        if ((int)key_to_freq.size() == capJam) {
            if (!key_freq_list.empty()) {
                // find smallest frequency bucket to evict from
                int min_freq = INT_MAX;
                for (const auto &p : key_freq_list) {
                    if (p.first < min_freq) min_freq = p.first;
                }
                auto &freq_list = key_freq_list[min_freq];
                // evict the oldest key in that frequency bucket (front is least recent within freq)
        ...
        if (rwtype == "Read") {
            ...
            writeHits++;
            // mark page as dirty on write so eviction stats stay accurate
...
void LFUCache::cachehits() {
	...
	if (result.is_open()) { 
		result <<  "LFU " << "CacheSize " << capJam << " calls " << calls << " hits " << hits << " hitRatio " << float(hits)/calls << " readHits " << readHits << " readHitRatio " << float(readHits)/calls << " writeHits " << writeHits << " writeHitRatio " << float(writeHits)/calls << " evictedDirtyPage " << evictedDirtyPage << "\n" ;			
	}
	result.close();
}
