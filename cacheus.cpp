...
CACHEUSCache::CACHEUSCache(int size) 
    : capZap(size), calls(0), hits(0), readHits(0), 
...
void CACHEUSCache::refer(long long int addr, string rwtype) {
    ...
    // Page miss
    // Use history to update expert weights based on who evicted this page (blends LRU/LFU)
    updateWeightsFromHistory(addr);

    // If cache is full, evict a page
    if ((int)table.size() < capZap) {
        insertNewPage(addr, rwtype);
    } else {
...
void CACHEUSCache::cachehits() {
	...
	if (result.is_open()) { 
		result <<  "CACHEUS " << "CacheSize " << capZap << " calls " << calls << " hits " << hits << " hitRatio " << float(hits)/calls << " readHits " << readHits << " readHitRatio " << float(readHits)/calls << " writeHits " << writeHits << " writeHitRatio " << float(writeHits)/calls << " evictedDirtyPage " << evictedDirtyPage << "\n" ;			
	}
	result.close();
}